...
function portalEmailHtml(contactName: string, portalUrl: string, newsletterName: string): string {
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
      <h2 style="color: #111827; margin: 0 0 12px;">Your Sponsor Portal</h2>
      <p style="color: #4b5563; font-size: 14px; line-height: 1.5;">
        Hi ${contactName}, here's your personal link to ${escapeHtml(newsletterName)} sponsor portal. Use it to submit listings, book ad dates, and check your status.
      </p>
...
export async function sendSponsorPortalEmail(
  ...
  try {
    ...
    await client.sendEmail({
      to,
      subject: `Your Sponsor Portal Link: ${workspace.name}`,
      htmlBody: portalEmailHtml(contactName, portalUrl, workspace.name),
    });
  } catch (err) {
